...
```bash
cd backend
# Install dependencies (including python-jose for JWT support)
pip install -e .
# Start the API
...
This project requires `python-jose` for JWT token handling in authentication. Make sure to install it:

```bash
# Install all dependencies including python-jose with cryptography support
