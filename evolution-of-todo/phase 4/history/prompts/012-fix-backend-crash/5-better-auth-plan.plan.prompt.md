...
command: sp.plan
labels: ["auth", "better-auth", "backend", "error-fix"]
links:
...
files:
  ...
  - F:\Hackathon II\evolution-of-todo\phase 2\specs\012-fix-backend-crash\contracts\better-auth-api-contract.md
tests: []
---

# Prompt

Create a corrective plan to fix the Better-Auth signup HTTP 500 error without changing the project's architecture.

Non-negotiable constraints:

- Authentication must remain Better Auth.
- Do NOT replace Better Auth with custom JWT, direct API calls, or custom middleware.
- Do NOT change the technology stack.

Tasks to include:

- Architecture Reconciliation:
  - Revert any changes that removed or bypassed Better Auth.
  - Restore Better-Auth-based `frontend/lib/auth.ts`, middleware, and auth flow.

- Backend Crash Diagnosis:
  - Trigger the signup request.
  - Capture the FastAPI traceback produced during the HTTP 500 error.
  - Identify the exact file, line, and function that fails.

- Failure Classification:
  - Determine whether the failure occurs in:
      - Better-Auth backend adapter
      - SQLModel user model
...
- Database Verification:
  ...
  - Ensure all auth tables exist and migrations are applied.

- Auth Logic Repair:
  - Fix the specific broken backend code path.
  - Ensure the Better-Auth signup endpoint returns a valid JSON response.

- End-to-End Validation:
  - Submit the signup form.
  - Confirm FastAPI returns HTTP 200/201.
  - Verify user creation and session establishment.

- Regression Protection:
  - Add error handling/logging so future auth errors return structured responses instead of HTTP 500.
